...
namespace AnhCF
{
    public partial class frmQuanLyCuaHang : Form
    {
        ...
        {      
            ...
            foreach (SanPhamDTO item in listSP)
            {
                ...
                if (item.anhsp != null && item.anhsp.Length > 0)
                {
                    try
                    {
                        using (MemoryStream mmr = new MemoryStream(item.anhsp))
                        {
                    ...
                    {
                        // Trường hợp dữ liệu ảnh bị lỗi, bạn có thể log hoặc gán ảnh mặc định
                        myUs[i].Anhsp = null;
                    }
                }
                else
        ...
        public void LamMoi()
        {
            gunatxtDonGia.Clear();
            gunatxtTenMon.Clear();
            gunacbbSize.SelectedIndex = -1;

            // Kiểm tra giá trị tối thiểu trước khi đặt giá trị cho Numeric Up Down
            if (gunanbrSoLuong.Minimum <= 0)
                gunanbrSoLuong.Value = Constants.NUMBER_DEFAULT_ZERO;

            if (gunanbrGiamGia.Minimum <= 0)
        ...
        private void guna2pnlMonAn_Paint(object sender, PaintEventArgs e)
        {

        }

        private void gunapnlMonAn_Paint(object sender, PaintEventArgs e)
        {
